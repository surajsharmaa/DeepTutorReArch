...
//(Modified Mihai Lintean Apr-14-2011)
// - handle the infinite loops while searching for paths
// - FIXED: the original function was not calculating correctly the LCS depth 
//	(the LCS depth should be calculated only from the current paths of the synsets, and not from all its paths to the root)   
...
// Case 3.
			...
			{
				int x = (p1.size() - p1.indexOf(minjoin));
				int y = (p2.size() - p2.indexOf(minjoin));
	...
	private ArrayList<ArrayList<ISynsetID>> paths(ISynset synset)
	{
		...
		ArrayList<ArrayList<ISynsetID>>			pathsToProcess = new ArrayList<ArrayList<ISynsetID>>();
		ArrayList<ISynsetID>			nodesVisited	=	new ArrayList<ISynsetID>();
		
		ArrayList<ISynsetID>			start	=	new ArrayList<ISynsetID>();
		start.add(synset.getID());
		
		pathsToProcess.add(start);
		
		
		while (pathsToProcess.size() > 0)
		{
			//extract one path from the paths to process list
			ArrayList<ISynsetID>	path	=	pathsToProcess.get(pathsToProcess.size()-1);
			pathsToProcess.remove(pathsToProcess.size()-1);
			...
			{
				{
					// LMC (4/15/2011) - do not allow search to go through a cycle a second time
					if (nodesVisited.contains(h)) continue;
					if (path.contains(h)) nodesVisited.add(h);
