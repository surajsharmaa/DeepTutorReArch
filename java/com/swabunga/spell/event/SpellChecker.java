...
//    String previousWord = null;
  ...
  /**
   * This method is called to automatically fix with best choice the spelling of the words that are returned
   * by the WordTokenizer.
   * Created by: Mihai Lintean (June 13, 2012)
   *
   * @param  tokenizer  The media containing the text to analyze.
   * @return The output is a string with the spell-checked words.
   */
  public final String autoSpelling(WordTokenizer tokenizer) {
    ...
    while (tokenizer.hasMoreWords() && !terminated) {
      ...
      if (!isCorrect(word)) {
          ...
          if (!isIgnored(word)) {
            ...
            if (autoReplaceWords.containsKey(word)) {
              tokenizer.replaceWord((String) autoReplaceWords.get(word));
              word = (String) autoReplaceWords.get(word);
            } else {
              //JMH Need to somehow capitalise the suggestions if
              //ignoreSentenceCapitalisation is not set to true
              //Fire the event.
              ...
              if (suggestions.size()>0) tokenizer.replaceWord(((Word) suggestions.get(0)).getWord());
            }
          }
        }
      } else {
        ...
        if (isSupposedToBeCapitalized(word, tokenizer)) {
          ...
          tokenizer.replaceWord((String) suggestion.get(0));
        }
      }
    }
